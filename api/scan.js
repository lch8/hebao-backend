...
export default async function handler(req) {
    ...
    try {
        ...
        // 🏃‍♂️ 第一棒：Gemini 提取纯净名字
        // ==========================================
        const geminiPrompt = `你是一个极其严谨的荷兰超市商品录入员。请提取图片中商品的【品牌名 + 核心品名】（荷兰语纯文本）。
        ⚠️ 必须严格遵守以下去重规则：
        1. 剔除所有重量、容量、尺寸信息（如 500g, 1L, 3 stuks）。
        2. 剔除所有促销、广告词汇（如 Nieuw, Bonus, Gratis, 1+1, Korting）。
        3. 严格按照 "品牌名 + 核心商品名" 输出。例如图片写着 'AH Pindakaas met stukjes 500g Bonus'，你只输出 'AH Pindakaas met stukjes'。
        如果看不清，直接回复'未识别'。`;
